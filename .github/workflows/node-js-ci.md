...
## Inputs
- `node-version` *(default `22`)* — Forwarded to `actions/setup-node`.
- `lint-script` *(default `lint`)* — npm script name used for linting before tests.
- `prettier-script` *(default `format:check`)* — npm script used to run Prettier in check mode.
- `badges-script` *(default `badges`)* — npm script that refreshes README/coverage badges.
- `test-script` *(default `test`)* — npm script that executes the main test suite.
